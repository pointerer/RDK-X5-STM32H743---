 ...
 * 临时 SDO 读取阶段诊断。
 *
 * 这些变量只供调试器观察，不参与协议判断，也不改变邮箱超时和控制流。
 * stage 含义：
 *   1：进入 ecx_SDOread
 *   2：未取得发送邮箱缓冲区
 *   3：开始发送邮箱请求
 ...
 *   7：函数准备返回
 */
volatile uint32 ecat_dbg_sdo_read_call_count;
volatile uint32 ecat_dbg_sdo_read_stage;
volatile int32 ecat_dbg_sdo_read_preflush_wkc;
volatile uint32 ecat_dbg_sdo_read_buffer_acquired;
volatile uint32 ecat_dbg_sdo_read_send_elapsed_us;
volatile int32 ecat_dbg_sdo_read_send_wkc;
volatile uint32 ecat_dbg_sdo_read_receive_elapsed_us;
volatile int32 ecat_dbg_sdo_read_receive_wkc;
volatile uint32 ecat_dbg_sdo_read_response_present;
volatile int32 ecat_dbg_sdo_read_return_wkc;

static uint32 ecx_sdo_diag_elapsed_us(ec_timet start_time)
{
   ...
   elapsed_us =
      ((current_time.tv_sec - start_time.tv_sec) * 1000000LL) +
      ((current_time.tv_nsec - start_time.tv_nsec) / 1000L);

   return (elapsed_us > 0) ? (uint32)elapsed_us : 0U;
}
                ...
                boolean CA, int *psize, void *p, int timeout)
{
   ...
   uint8 cnt, toggle;
   boolean NotLast;
   ec_timet diag_start_time;

   ecat_dbg_sdo_read_call_count++;
   ecat_dbg_sdo_read_stage = 1U;
   ecat_dbg_sdo_read_preflush_wkc = 0;
   ecat_dbg_sdo_read_buffer_acquired = 0U;
   ecat_dbg_sdo_read_send_elapsed_us = 0U;
   ecat_dbg_sdo_read_send_wkc = 0;
   ecat_dbg_sdo_read_receive_elapsed_us = 0U;
   ecat_dbg_sdo_read_receive_wkc = 0;
   ecat_dbg_sdo_read_response_present = 0U;
   ecat_dbg_sdo_read_return_wkc = 0;

   MbxIn = NULL;
   MbxOut = NULL;
   wkc = ecx_mbxreceive(context, slave, &MbxIn, 0);
   ecat_dbg_sdo_read_preflush_wkc = (int32)wkc;
   MbxOut = ecx_getmbx(context);
   if (!MbxOut)
   {
      ecat_dbg_sdo_read_stage = 2U;
      ecat_dbg_sdo_read_return_wkc = (int32)wkc;
      return wkc;
   }
   ecat_dbg_sdo_read_buffer_acquired = 1U;
   ec_clearmbx(MbxOut);
   SDOp = (ec_SDOt *)MbxOut;
   ...
   ecat_dbg_sdo_read_stage = 3U;
   diag_start_time = osal_current_time();
   wkc = ecx_mbxsend(context, slave, MbxOut, EC_TIMEOUTTXM);
   ecat_dbg_sdo_read_send_elapsed_us =
      ecx_sdo_diag_elapsed_us(diag_start_time);
   ecat_dbg_sdo_read_send_wkc = (int32)wkc;
   ecat_dbg_sdo_read_stage = 4U;
   MbxOut = NULL;
   if (wkc > 0) /* succeeded to place mailbox in slave ? */
   {
      ...
      MbxIn = NULL;
      ecat_dbg_sdo_read_stage = 5U;
      diag_start_time = osal_current_time();
      do
      {
         wkc = ecx_mbxreceive(context, slave, &MbxIn, timeout);
      } while ((wkc > 0) && !MbxIn);
      ecat_dbg_sdo_read_receive_elapsed_us =
         ecx_sdo_diag_elapsed_us(diag_start_time);
      ecat_dbg_sdo_read_receive_wkc = (int32)wkc;
      ecat_dbg_sdo_read_response_present = (MbxIn != NULL) ? 1U : 0U;
      ecat_dbg_sdo_read_stage = 6U;
      aSDOp = (ec_SDOt *)MbxIn;
