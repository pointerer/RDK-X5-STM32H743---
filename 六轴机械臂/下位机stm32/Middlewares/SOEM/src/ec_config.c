...
/** 初始化 SOEM 上下文：清空从站和组信息、重置 EEPROM 缓存状态，并初始化各组逻辑地址及邮箱队列。 */
static int ecx_init_context(ecx_contextt *context)
{
   int lp;
   int cleanup_group;

   /* A repeated configuration must not lose previously allocated queue mutexes. */
   ...
   for (lp = 0; lp < EC_MAXGROUP; lp++)
   {
      ...
      if (ecx_initmbxqueue(context, lp) != 0)
      {
         for (cleanup_group = 0; cleanup_group < lp; cleanup_group++)
         {
            ec_mbxqueuet *mbxqueue = &(context->grouplist[cleanup_group].mbxtxqueue);
            osal_mutex_destroy(mbxqueue->mbxmutex);
            mbxqueue->mbxmutex = NULL;
         }
         return EC_ERROR;
