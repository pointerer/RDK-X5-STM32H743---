 ...
 * 临时循环邮箱发送队列诊断。
 *
 * 这些字段只供调试器读取，不参与邮箱队列或超时判断。
 * ticket=-2 表示尚未尝试入队，ticket=-1 表示入队失败。
 * ticketloc/queue_state/remove/listcount 是超时回收前、在原队列锁内取得的快照。
 */
volatile uint32 ecat_dbg_mbxsend_call_count;
volatile uint32 ecat_dbg_mbxsend_slave;
volatile uint32 ecat_dbg_mbxsend_add_attempts;
volatile int32 ecat_dbg_mbxsend_ticket;
volatile uint32 ecat_dbg_mbxsend_done_polls;
volatile int32 ecat_dbg_mbxsend_ticketloc;
volatile int32 ecat_dbg_mbxsend_queue_state;
volatile int32 ecat_dbg_mbxsend_remove;
volatile int32 ecat_dbg_mbxsend_listcount;
volatile int32 ecat_dbg_mbxsend_expire_result;
volatile int32 ecat_dbg_mbxsend_return_wkc;
...
int ecx_init(ecx_contextt *context, const char *ifname)
{
   ...
   if (ecx_initmbxpool(context) != 0)
   {
      return 0;
   }

   rval = ecx_setupnic(&context->port, ifname, FALSE);
   if (rval <= 0)
   {
...
void ecx_close(ecx_contextt *context)
{
   ...
   for (int lp = 0; lp < EC_MAXGROUP; lp++)
   {
      ...
      if (mbxqueue->mbxmutex)
      {
         ...
         mbxqueue->mbxmutex = NULL;
      }
   }

   if (context->mbxpool.mbxmutex)
   {
      osal_mutex_destroy(context->mbxpool.mbxmutex);
      context->mbxpool.mbxmutex = NULL;
   }
...
{
   /* 邮箱缓冲区在 context 中按环形空闲列表管理。 */
   ec_mbxpoolt *mbxpool = &context->mbxpool;
   /* 互斥锁用于保护邮箱缓冲区的申请和释放。 */
   mbxpool->mbxmutex = osal_mutex_create();
   if (mbxpool->mbxmutex == NULL)
   {
      return EC_ERROR;
   }
   /* 初始化时，所有邮箱缓冲区槽位都是空闲的。 */
...
int ecx_initmbxqueue(ecx_contextt *context, uint8 group)
{
   ...
   mbxqueue->mbxmutex = osal_mutex_create();
   if (mbxqueue->mbxmutex == NULL)
   {
      return EC_ERROR;
   }
   mbxqueue->listhead = 0;
...
int ecx_mbxdonequeue(ecx_contextt *context, uint16 slave, int ticket)
{
   ...
   if ((ticket >= 0) && (ticket < EC_MBXPOOLSIZE))
   {
      ...
      ec_mbxqueuet *mbxqueue = &(context->grouplist[group].mbxtxqueue);
      osal_mutex_lock(mbxqueue->mbxmutex);
      ticketloc = mbxqueue->mbxticket[ticket];
      if ((ticketloc >= 0) && (ticketloc < EC_MBXPOOLSIZE) &&
          (mbxqueue->mbxstate[ticketloc] == EC_MBXQUEUESTATE_DONE))
      {
         mbxqueue->mbxremove[ticketloc] = 1;
...
/* Mailbox finalization result at the send timeout boundary. */
#define EC_MBX_FINALIZE_INVALID  0
#define EC_MBX_FINALIZE_EXPIRED  1
#define EC_MBX_FINALIZE_DONE     2

/** Finalize a mailbox in the queue at the send timeout boundary.
 * @param[in]  context        context struct
 * @param[in]  slave          Slave number
 * @param[in]  ticket         Ticket number of the mailbox
 * @return EC_MBX_FINALIZE_DONE if the mailbox completed before finalization,
 *         EC_MBX_FINALIZE_EXPIRED if a pending/failed mailbox was expired,
 *         or EC_MBX_FINALIZE_INVALID if the ticket is invalid.
 */
int ecx_mbxexpirequeue(ecx_contextt *context, uint16 slave, int ticket)
{
   int retval = EC_MBX_FINALIZE_INVALID;
   int ticketloc;
   uint8 state;
   if ((ticket >= 0) && (ticket < EC_MBXPOOLSIZE))
   {
      ...
      osal_mutex_lock(mbxqueue->mbxmutex);
      ticketloc = mbxqueue->mbxticket[ticket];
      if (((uint32)slave == ecat_dbg_mbxsend_slave) &&
          ((int32)ticket == ecat_dbg_mbxsend_ticket))
      {
         ecat_dbg_mbxsend_ticketloc = (int32)ticketloc;
         ecat_dbg_mbxsend_listcount = (int32)mbxqueue->listcount;
         if ((ticketloc >= 0) && (ticketloc < EC_MBXPOOLSIZE))
         {
            ecat_dbg_mbxsend_queue_state =
               (int32)mbxqueue->mbxstate[ticketloc];
            ecat_dbg_mbxsend_remove =
               (int32)mbxqueue->mbxremove[ticketloc];
         }
      }
      if ((ticketloc >= 0) && (ticketloc < EC_MBXPOOLSIZE) &&
          (mbxqueue->mbxstate[ticketloc] > EC_MBXQUEUESTATE_NONE))
      {
         ...
         mbxqueue->mbxremove[ticketloc] = 1;
         mbxqueue->mbxticket[ticket] = -1;
         if (state == EC_MBXQUEUESTATE_DONE)
         {
            retval = EC_MBX_FINALIZE_DONE;
         }
         else
         {
            retval = EC_MBX_FINALIZE_EXPIRED;
         }
      }
      osal_mutex_unlock(mbxqueue->mbxmutex);
   }
            ...
            if ((mbxl > 0) && (mbx = ecx_getmbx(context)))
            {
               ...
               if (wkc > 0)
               {
                  ...
                  else if ((mbxh->mbxtype & 0x0f) == ECT_MBXT_COE) /* CoE response? */
                  {
                     ...
                     {
                        osal_mutex_lock(context->grouplist[group].mbxtxqueue.mbxmutex);
                        ...
                        osal_mutex_unlock(context->grouplist[group].mbxtxqueue.mbxmutex);
                     }
                  }
...
int ecx_mbxouthandler(ecx_contextt *context, uint8 group, int limit)
{
   ...
   while ((limitcnt < limit) && listcount)
   {
      ...
      /* Snapshot one queue entry, but never hold the queue lock during network I/O. */
      osal_mutex_lock(mbxqueue->mbxmutex);
      ticketloc = mbxqueue->listtail;
      state = mbxqueue->mbxstate[ticketloc];
      slave = mbxqueue->mbxslave[ticketloc];
      mbx = mbxqueue->mbx[ticketloc];
      remove = mbxqueue->mbxremove[ticketloc];
      osal_mutex_unlock(mbxqueue->mbxmutex);
 ...
 *
 * @return Total count of processed incoming and outgoing mailboxes.
 */
int ecx_mbxhandler(ecx_contextt *context, uint8 group, int limit)
{
   ...
   incnt = ecx_mbxinhandler(context, group, limit);
   outcnt = ecx_mbxouthandler(context, group, (limit - incnt));
   return incnt + outcnt;
}
...
int ecx_mbxsend(ecx_contextt *context, uint16 slave, ec_mbxbuft *mbx, int timeout)
{
   ...
   int wkc, ticket, expire_result;
   osal_timert timer;
   ec_slavet *slavelist = &(context->slavelist[slave]);

   ecat_dbg_mbxsend_call_count++;
   ecat_dbg_mbxsend_slave = (uint32)slave;
   ecat_dbg_mbxsend_add_attempts = 0U;
   ecat_dbg_mbxsend_ticket = -2;
   ecat_dbg_mbxsend_done_polls = 0U;
   ecat_dbg_mbxsend_ticketloc = -1;
   ecat_dbg_mbxsend_queue_state = -1;
   ecat_dbg_mbxsend_remove = -1;
   ecat_dbg_mbxsend_listcount = -1;
   ecat_dbg_mbxsend_expire_result = -1;
   ecat_dbg_mbxsend_return_wkc = 0;
   ...
   if (slavelist->mbxhandlerstate == ECT_MBXH_CYCLIC)
   {
      ...
      if (mbxl > 0)
      {
         do
         {
            ecat_dbg_mbxsend_add_attempts++;
            ticket = ecx_mbxaddqueue(context, slave, mbx);
            ecat_dbg_mbxsend_ticket = (int32)ticket;
            if (ticket >= 0)
            {
               mbx = NULL;
               do
               {
                  ecat_dbg_mbxsend_done_polls++;
                ...
                if (wkc <= 0)
                {
                   expire_result = ecx_mbxexpirequeue(context, slave, ticket);
                   ecat_dbg_mbxsend_expire_result = (int32)expire_result;
                   if (expire_result == EC_MBX_FINALIZE_DONE)
                   {
                      wkc = 1;
                   }
                   else if (expire_result == EC_MBX_FINALIZE_INVALID)
                   {
                      EC_PRINT("finalize mailbox failed\n\r");
                   }
                }
            }
   ...
   if (mbx) ecx_dropmbx(context, mbx);
   ecat_dbg_mbxsend_return_wkc = (int32)wkc;
   return wkc;
}
...
int ecx_mbxreceive(ecx_contextt *context, uint16 slave, ec_mbxbuft **mbx, int timeout)
{
   ...
   if (slavelist->mbxhandlerstate == ECT_MBXH_CYCLIC)
   {
      osal_mutext *responsemutex =
         context->grouplist[slavelist->group].mbxtxqueue.mbxmutex;
      osal_timer_start(&timer, timeout);
      wkc = 0;
      do
      {
         osal_mutex_lock(responsemutex);
         ...
         osal_mutex_unlock(responsemutex);
         if ((wkc == 0) && (slavelist->soembxinfull == TRUE))
         {
         ...
         else if ((wkc == 0) && (slavelist->foembxinfull == TRUE))
         {
         ...
         else if ((wkc == 0) && (slavelist->eoembxinfull == TRUE))
         {
      ...
      /* The cyclic handler may have cached the CoE response while this lower
       * priority caller was preempted at the timeout boundary. Claim it once
       * more under the same mutex used by the handler before reporting a
       * receive timeout. */
      if (wkc <= 0)
      {
         osal_mutex_lock(responsemutex);
